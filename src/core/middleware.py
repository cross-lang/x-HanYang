...
提供请求 ID 分配、请求日志记录、全局异常兜底等通用中间件。
通过 logger.contextualize() 实现 request_id 在整个请求生命周期内的自动注入。
"""
...
class RequestIDMiddleware(BaseHTTPMiddleware):
    ...
    通过 logger.contextualize() 将 request_id 注入 loguru 上下文，
    使该请求作用域内的所有日志自动携带 request_id 字段。
    """
    ...
    async def dispatch(self, request: Request, call_next: Callable[[Request], Awaitable[Response]]) -> Response:
        ...
        # 将 request_id 注入 loguru 上下文，后续所有日志自动携带
