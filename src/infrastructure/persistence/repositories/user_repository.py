...
class SqlUserRepository(UserRepository):
    ...
    async def find_by_email(self, email: str) -> User | None:
        """根据邮箱查找用户（排除软删除）。"""
        stmt = select(UserTable).where(UserTable.email == email, UserTable.deleted_at.is_(None))
        row = (await self._session.execute(stmt)).scalars().first()
        return self._to_domain(row) if row else None

    async def find_by_username(self, username: str) -> User | None:
        """根据用户名查找用户（排除软删除）。"""
        stmt = select(UserTable).where(UserTable.username == username, UserTable.deleted_at.is_(None))
        row = (await self._session.execute(stmt)).scalars().first()
        return self._to_domain(row) if row else None
    ...
    async def save(self, user: User) -> None:
        ...
        if user.id and user.id > 0:
            # 更新
            existing = await self._session.get(UserTable, user.id)
            if existing:
    ...
    async def delete(self, id: int) -> bool:
        ...
        row = await self._session.get(UserTable, id)
        if row is None:
            return False
